...
| `services/` | Peripheral external service adapters. | `DataExportService.ts` (exports/imports transactions backup), `yahooFinanceService.ts` (Automated real-time stock quote & historical streak data engine supporting NSE `.NS`, BSE `.BO`, and global equities with concurrent batch fetching). |
...
### C. Ticker Price Synchronization & Live Market Data Engine
*   **Behavior**:
    1. **On-Demand & Real-Time Ingestion**: The application utilizes [yahooFinanceService.ts](file:///Users/akashsharma/Documents/Gainbase/services/yahooFinanceService.ts) to query real-time market data across all NSE (`.NS`), BSE (`.BO`), and global equities.
    2. **Multi-Source P/E, Market Cap & Sector Engine**: Combines Yahoo Finance fundamentals with zero-cookie fallbacks (`fetchLivePE` and `fetchLiveMarketCap`) and local sector inference (`inferSector`), ensuring **Market Cap** (e.g. `₹7,117.91 Cr`), **P/E ratio** (e.g. `9.04`), and **Sector** (e.g. *Bank*, *IT*, *Steel/ Iron Products*, *Refineries*, *FMCG*, etc.) are **100% guaranteed** to resolve on all mobile platforms.
    3. **Real-Time USD to INR Currency Conversion (`fetchUsdInrRate`)**: Real-time conversion of global / US equities (e.g. `AAPL`, `MSFT`, `GOOGL`, `AMZN`, `NVDA`, `TSLA`, `META`, `NFLX`) into Indian Rupees (`₹`) using live exchange rates from Yahoo Finance (`USDINR=X`). Automatically converts Current Price, 52W High/Low, Yesterday Close, and Market Cap into INR.
    4. **High-Resolution Stock Logo Engine (`getCompanyLogoUrl`)**: Dynamically resolves crystal-clear, high-resolution official company logos across the application (Stock Details Hero Card, Explore search & watchlist, Top Movers, Holdings/Allocations, and Transaction Pickers). Uses a multi-tier high-res resolution engine:
       - *Tier 1 (High-Res Financial Stock CDN)*: Dedicated Parqet stock asset CDN (`https://assets.parqet.com/logos/symbol/${symbol}?format=png`) delivering transparent, high-DPI official corporate logos for Indian NSE equities (e.g. `DCBBANK.NS`, `TATASTEEL.NS`, `SBIN.NS`, `RELIANCE.NS`, `ZOMATO.NS`) and US equities (`AAPL`, `MSFT`, `NVDA`, `GOOGL`, etc.).
       - *Tier 2 (Vector Unavatar Delivery)*: Corporate vector/PNG unavatar matching extracted from `assetProfile.website` or `KNOWN_STOCK_DOMAINS`.
       - *Tier 3 (Automated Cache Upgrade)*: Store automatically migrates any legacy low-res favicons in local storage to high-res assets on refresh.
    5. **Master Stock & ETF Dataset with Fuzzy Search**: [constants/NSE_COMPANIES.ts](file:///Users/akashsharma/Documents/Gainbase/constants/NSE_COMPANIES.ts) contains 2,600+ verified companies, Indian Gold/Silver/Index/Sectoral ETFs (`SETFGOLD`, `GOLDBEES`, `NIFTYBEES`, `BANKBEES`, `SILVERBEES`, `CPSEETF`, `MON100`, `MAFANG`, etc.), US ETFs (`SPY`, `QQQ`, `VOO`, `VTI`, `SMH`), and popular Mutual Funds. `searchMasterStocks(query)` delivers sub-millisecond, multi-word matching across company names, aliases (e.g. *"SBI Gold ETF"*, *"SBI ETF Gold"*, *"Gold BeES"*, *"Nifty BeES"*, *"Parag Parikh"*, *"Apple"*), and ticker symbols with instant logo rendering.
    6. **Dynamic Real-Time Yahoo Finance Search Engine (`searchYahooTickers`)**: Integrated in both the **Explore Screen** and **Add Transaction Asset Selector Modal**. Employs a 300ms debounced live query against Yahoo Finance's global search API, dynamically resolving equities, ETFs, and Mutual Funds anywhere in the world and merging with offline master records seamlessly.
    7. **Active Holdings & Watchlist Refresh**: On app load or refresh in `store/usePortfolioStore.ts` via `fetchTickers()`, the engine collects all active symbols from user transactions, watchlist, and cached store, performing throttled concurrent batch fetches against Yahoo Finance.
    8. **Background Cloud Caching**: Fresh quotes (prices, 52W High/Low, PE, Market Cap, Logo, and day changes) are merged into local state and asynchronously upserted to the Supabase database table `public.tickers`, keeping the cloud backend warm.
    9. **On-Demand Ticker Lookups (`fetchSingleTicker`)**: When selecting any stock in `add-transaction.tsx`, `explore.tsx`, or `stock-details/[symbol].tsx`, the app fetches up-to-the-second quotes live from Yahoo Finance and caches them in Zustand and Supabase.
